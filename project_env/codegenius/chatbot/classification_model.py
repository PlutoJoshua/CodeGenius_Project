...
def Google_gemini(user_input, api_key):
    ...
    user_prompt가 파이썬과 관련된 질문 중에서 "파이썬 정의, 파이썬 기초, 조건문, 반복문, 변수, 내장함수, ARIMA, 다중공선성, 선형회귀, 기본 패키지, 개행문자, 문자열 포맷, tuple, list(리스트), set, 넘파이, 판다스, 데이터프레임, 배열, 행렬, 시각화, matplotlib, seaborn, plotly, 기초통계, 가설검정, 회귀분석, 상관분석, 차이분석, 그룹화, 탐색적 데이터 분석, 데이터 전처리, 데이터의 종류, 데이터의 특성"과 관련된 질문인지 아닌지 분류해주세요.
